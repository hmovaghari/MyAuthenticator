    ...
    public partial class frmMain : Form
    {
        ...
        private void btnShowPassword_Click(object sender, EventArgs e)
        {
            ...
            if (currentPassword != null)
            {
                ...
                currentPassword.PasswordString = totp.ComputeTotp();
                grdPassword.Refresh();
                ...
                AddClearPasswordEvent(ClearPasswordEvent(currentPassword));
                timerClearPassword.Start();
            }
        }

        private EventHandler ClearPasswordEvent(PasswordDTO currentPassword)
        {
            return new EventHandler(delegate (object timerSender, EventArgs timerEventArgs)
            {
                currentPassword.PasswordString = PasswordDTO.PasswordStar;
                grdPassword.Refresh();
                timerClearPassword.Stop();
                timerClearPassword.Enabled = false;
                RemoveClearPasswordEvent(ClearPasswordEvent(currentPassword));
            });
        }

        public void AddClearPasswordEvent(EventHandler eventHandler)
        {
            timerClearPassword.Tick += eventHandler;
        }

        public void RemoveClearPasswordEvent(EventHandler eventHandler)
        {
            timerClearPassword.Tick -= eventHandler;
        }

        private ThreadStart PasswordClear(PasswordDTO currentPassword, int remainingSeconds)
        {
            var timeSpan = new TimeSpan(0, 0, remainingSeconds);
            Thread.Sleep(timeSpan);
            currentPassword.PasswordString = null;
            grdPassword.Refresh();
            return new ThreadStart(NullThread);
        }

        private void NullThread()
        {
